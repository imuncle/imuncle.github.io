...
>费米能级是费米分布里面的一个参考能级，不是实际存在的。等于费米能级的一半填充一半不填充
...
1. 杂质半导体的载流子浓度随温度由低到高的变化关系,存在三个区域：`部分电离`、`非本征区`、`本征区`，浓度越来越高
...
1. 半导体中主要有两种散射的机制影响载流子的迁移率：`晶格散射`（也叫声子散射），有利于迁移，与温度是负指数关系；`电离杂质散射`
...
1. 零偏平衡状态的时候，PN结空间电荷区的电势是`n`区高
>能级能量高低对电子，电势高低与能级高低相反
...
1. 影响肖特基势垒高低的非理想因素包括两个：`非镜像力`和`界面态`
1. 二维电子气
...
1. 双极型晶体三极管影响共基级电流增益的三个因子为`发射极注入效率`、`基区输运系数`、`复合系数`
...
1. 光电晶体管的频率响应受限于BC结电容，那么BC结电容会因`米勒`效应而成倍增加
...
>饱和速度：载流子漂移速度不再随外加电场增加而变化时所达到的速度最大值
原因：对砷化镓，随外加电压增大，低能谷电子可能被散射至高能谷中，使有效质量增加，迁移率变小
...
不同：
①电流输运机理不同（PN结由少子扩散决定，肖特基是多子通过热电发射跃过内建电势差而形成）
②肖特基势垒二极管的理想反向饱和电流大几个数量级
③肖特基二极管有效开启电压低
④开关特性不同
